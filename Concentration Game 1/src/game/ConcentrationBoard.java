...
public class ConcentrationBoard {
    ...
    public ConcentrationCard getCard(int row, int col) throws ConcentrationException {
        if (row < 0 || row >= this.DIM || col < 0 || col > this.DIM) {
            throw new ConcentrationException("Coordinate out of range: (" + row + ", " + col + ")");
        }
    ...
    public CardMatch reveal(int row, int col) throws ConcentrationException {
        ...
        } else if (row < 0 || row >= this.DIM || col < 0 || col > this.DIM) {
            throw new ConcentrationException("Coordinate out of range: (" + row + ", " + col + ")");
        } else if (!this.board[row][col].isHidden()) {
            throw new ConcentrationException("Card is not hidden: (" + row + ", " + col + ")");
        }

        // grab the card and reveal it
        ...
        if (this.revealedCard == null) {
            // if this is the first card there isn't a match yet
            this.revealedCard = card;
            match = new CardMatch(card, null, false);
        } else {
            if (this.revealedCard.equals(card)) {
                // if this is the second card and there is a match, count it
                match = new CardMatch(this.revealedCard, card, true);
                ++this.matches;
            } else {
                // if there isn't a match, hide both revealed cards and make note of it
                match = new CardMatch(this.revealedCard, card, false);
                card.hide();
